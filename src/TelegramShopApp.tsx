...
export default function TelegramShopApp() {
  ...
  function addToCart(product: Product, quantity: number) {
    ...
    void trackStoreEvent({ type: 'add_to_cart', productId: product.id, quantity, value: product.price * quantity, userId: user?.id ? `tg:${user.id}` : undefined });
  }
  ...
  async function checkoutSuccess(order: Order) {
    for (const item of order.items) {
      void trackStoreEvent({ type: 'purchase', productId: item.productId, quantity: item.quantity, value: item.price * item.quantity, userId: user?.id ? `tg:${user.id}` : undefined, metadata: { orderId: order.id, source: 'telegram_webapp' } });
    }
    saveCart([]);
    try {
